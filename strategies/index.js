...
function processLoginCallback(req, username, password, done) {
	...
	User.findOne({
		...
		.then(function (user) {
			if (!user) return done(null, false, { foundUser: false })
			if (user.password !== password) return done(null, false, { validPassword: false })
...
function processSignupCallback(req, username, password, done) {
	User.findOne({
		...
		.then(function (user) {
			if (user) return done(null, false, { usernameExists: true });
			else {
